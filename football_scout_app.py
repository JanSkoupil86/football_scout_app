...
st.markdown(
    ...
    "Caching, robust parsing, ALL filters, column ordering, 2-decimal rounding, "
    "downloads, and a radar chart are included."
)
...
def multiselect_all(
    ...
    *,
    ...
    """Multiselect with an 'ALL' option; returns resolved selection & flag."""
    all_token = "ALL"
...
# Columns we never coerce to numeric
NON_FEATURE_COLUMNS = {
...
def parse_market_value(series: pd.Series) -> pd.Series:
    """Return market value in millions of EUR as float; handles 12.5m / 800k / 12,000,000."""
...
def coerce_numeric(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Coerce other numeric-looking columns
...
def reorder_pills(items: list, *, key: str, direction: str = "horizontal") -> list:
    """Silent drag-and-drop support if a compatible component is installed; otherwise passthrough."""
    try:
...
if uploaded is None:
    st.info("Please upload your CSV (must include 'Player', 'Team', 'Main Position', 'Age', 'League').")
    st.stop()
...
selected_leagues, _ = multiselect_all("League(s)", leagues, default_all=True, help="Pick leagues or ALL")
filtered = df[df["League"].isin(selected_leagues)].copy()
if filtered.empty:
    st.warning("No players found for selected leagues.")
    st.stop()
...
if mv_col is not None:
    mv_series = filtered[mv_col].replace([np.inf, -np.inf], np.nan).dropna()
    if not mv_series.empty:
        mv_min, mv_max = float(mv_series.min()), float(mv_series.max())
        mv_range = st.sidebar.slider(
            "Market value range" + (" (M€)" if mv_col == "Market value (M€)" else ""),
            float(np.floor(mv_min)),
            float(np.ceil(mv_max)),
...
# ---------------------------
# Player Performance Visualization (scatter)
# ---------------------------
...
# ---------------------------
# Compare Selected Players (table + radar)
# ---------------------------
...
if compare_players:
    ...
    if comp_metrics:
        ...
        # Radar chart normalization against all currently filtered players
